...
if(!class_exists('VC_Ultimate_Parallax')){
	class VC_Ultimate_Parallax{
		...
		public static function parallax_shortcode($atts, $content){
			...
			$bg_type = $bg_image = $bg_image_new = $bsf_img_repeat = $parallax_style = $video_opts = $video_url = $video_poster = $bg_image_size = $bg_image_posiiton = $u_video_url = $parallax_sense = $bg_cstm_size = $bg_override = $bg_img_attach = $u_start_time = $u_stop_time = $layer_image = $css = "";
			...
			elseif($bg_type == "image"){
				...
				$html .= '<div class="upb_content_video" data-ultimate-video="'.$video_url.'" data-ultimate-video-muted="'.$muted.'" data-ultimate-video-loop="'.$loop.'" data-ultimate-video-poster="'.$v_img[0].'" data-ultimate-video-autoplay="autoplay" data-bg-override="'.$bg_override.'" data-upb-overlay-color="'.$overlay_color.'" data-upb-bg-animation="'.$bg_animation.'"></div>';
			}
			elseif ($bg_type=='u_iframe') {
				wp_enqueue_script('jquery.tublar',plugins_url('../assets/js/tubular.js',__FILE__),'1.0',array('jQuery'),true);
				$v_opts = explode(",",$video_opts);
				$v_img = wp_get_attachment_image_src($video_poster,'full');
				if(is_array($v_opts)){
					foreach($v_opts as $opt){
			...
			if(function_exists('vc_add_param')){
				vc_add_param('vc_row',array(
						...
						"value" => array(
							...
							__("Image","upb_parallax") => "image",
							__("Hosted Video","upb_parallax") => "video",
							__("YouTube Video","upb_parallax") => "u_iframe",
							__("Gradient","upb_parallax") => "grad",
							__("No","upb_parallax") => "no_bg",
							),
						"description" => __("Select what kind of background would you like to set for this row.", "upb_parallax"),
						"group" => $group_name,
					)
				);
				vc_add_param('vc_row',array(
						...
						"param_name" => "bg_grad",
						"description" => __("Select Gradient from gradient points.", "upb_parallax"),
				...
				vc_add_param('vc_row',array(
						"type" => "checkbox",
						"class" => "",
						"heading" => __("Video Play Options", "upb_parallax"),
				...
				vc_add_param('vc_row',array(
						"type" => "attach_image",
						"class" => "",
						"heading" => __("Placehoder Image for the video", "upb_parallax"),
						"param_name" => "video_poster",
						"value" => "",
						"description" => __("Select the placehoder image that will be display while video is loading.", "upb_parallax"),
						"dependency" => Array("element" => "bg_type","value" => array("video","u_iframe")),
						"group" => $group_name,
					)
				);
				vc_add_param('vc_row',array(
						"type" => "textfield",
						"class" => "",
						"heading" => __("Video URL", "upb_parallax"),
						"param_name" => "video_url",
						"value" => "",
						"description" => __("Enter your video URL. You can upload a video through <a href='/wp-admin/media-new.php' target='_blank'>WordPress Media Library</a>, if not done already.", "upb_parallax"),
				...
				vc_add_param('vc_row',array(
						"type" => "number",
						"class" => "",
						"heading" => __("Start Time in seconds", "upb_parallax"),
				...
				vc_add_param('vc_row',array(
						"type" => "number",
						"class" => "",
						"heading" => __("Stop Time in seconds", "upb_parallax"),
						"param_name" => "u_stop_time",
						"value" => "",
						"description" => __("Enter time in seconds where video ends.", "upb_parallax"),
						"dependency" => Array("element" => "bg_type","value" => array("u_iframe")),
						"group" => $group_name,
					)
				);/*
				...
				vc_add_param('vc_row',array(
						"type" => "dropdown",
						"class" => "",
						"heading" => __("Background Override", "upb_parallax"),
				...
				jQuery(document).ready(function(){					
						function gradient_pre_defined(){
							jQuery('.vc_ug_control').each(function(){
								...
								jQuery('.colorpicker').css('z-index','99999');
							})
						}	
		...
		function admin_scripts(){
			wp_enqueue_script('jquery.colorpicker',plugins_url('../admin/js/jquery.colorpicker.js',__FILE__),'1.0',array('jQuery'),true);
			wp_enqueue_script('jquery.classygradient',plugins_url('../admin/js/jquery.classygradient.min.js',__FILE__),'1.0',array('jQuery'),true);			
		...
		static function front_scripts(){
			...
			wp_enqueue_script('jquery.shake',plugins_url('../assets/js/jparallax.js',__FILE__),'1.0',array('jQuery'),true);
			wp_enqueue_script('jquery.vhparallax',plugins_url('../assets/js/jquery.vhparallax.js',__FILE__),'1.0',array('jQuery'),true);			
