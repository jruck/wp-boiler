...
if(!class_exists('AIO_Interactive_Banners')) 
{
	class AIO_Interactive_Banners
	{
		...
		function banner_shortcode($atts)
		{
			...
			$banner_title = $banner_desc = $banner_icon = $banner_image = $banner_link = $banner_link_text = $banner_style = $banner_bg_color = $el_class = $animation = $icon_disp = $link_opts = $banner_title_location = '';
			...
			if($icon_disp == "with_description" || $icon_disp == "both"){
				...
				$output .= "\n\t\t".'<p class="bb-description">'.$banner_desc.'</p>';
			}
			if($link_opts == "more")
